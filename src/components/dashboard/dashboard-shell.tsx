...
import {
  LayoutDashboard, Map, Brain, Wrench, LogOut, Menu, X, ChevronLeft, User, Sun, Moon,
} from "lucide-react";
...
const navItems = [
  ...
  { title: "Pathways", href: "/pathways", icon: Map },
  { title: "Study Cards", href: "/study-cards", icon: Brain },
  { title: "Toolbox", href: "/toolbox", icon: Wrench },
];

// Terima profile sebagai props, bukan nge-fetch sendiri
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      >
        ...
        <div className="flex items-center justify-between h-16 px-4 border-b border-border/50">
          <Link href="/dashboard" className="flex items-center gap-2">
            <div className="relative h-9 w-9 overflow-hidden rounded-xl shrink-0">
              <Image src="/img/logo.jpeg" alt="EduFlow Logo" fill sizes="36px" className="object-cover" priority />
            </div>
            {!collapsed && (
        ...
        {/* User Card (Menggunakan props profil dari server) */}
        <div className="border-t border-border/50 p-3">
          <div className={cn("flex items-center gap-3 p-3 rounded-xl bg-accent/50", collapsed && "justify-center")}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <div className="relative h-7 w-7 overflow-hidden rounded-lg">
                <Image src="/img/logo.jpeg" alt="EduFlow Logo" fill sizes="28px" className="object-cover" priority />
              </div>
              <span className="text-sm font-bold">EduFlow</span>
            </div>
          </div>
          <button onClick={toggleTheme} className="text-muted-foreground hover:text-foreground transition-colors" title={theme === "dark" ? "Switch to Light Mode" : "Switch to Dark Mode"}>
